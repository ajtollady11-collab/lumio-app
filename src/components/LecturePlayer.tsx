    ...
    async function generate() {
      try {
        const res = await fetch("/api/generate", {
          ...
          // Always use the typed topic as the subject so Claude knows the actual topic
          // e.g. if user types "Macbeth", subject should be "Macbeth" not "Computer Science"
          body: JSON.stringify({
            mode: "lecture",
            subject: topicInput,
            topic: topicInput,
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0d0f1a] text-white">
      <div className="flex items-center justify-between border-b border-white/10 px-6 py-3">
        <button onClick={() => { stop(); router.back(); }} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm text-white/60 hover:bg-white/10">
          ← Exit lecture
        </button>
        <div className="text-sm font-medium text-white/80">{lecture?.title ?? (topicInput || subject)}</div>
        <div className="text-sm text-white/40">{lecture ? `${slideIndex + 1} / ${lecture.slides.length}` : ""}</div>
      </div>

      <div className="h-1 bg-white/10">
        <div className="h-full bg-gradient-to-r from-[var(--indigo-2)] to-[var(--indigo)] transition-all duration-500" style={{ width: `${progress}%` }} />
      </div>

      <div className="flex flex-1 overflow-hidden">
        <div className="flex w-64 shrink-0 flex-col items-center justify-end border-r border-white/10 bg-[#12142a] pb-8 pt-6">
          <TeacherAvatar name={teacherName} isSpeaking={voiceState === "playing"} phase={phase} />
          <div className="mt-4 text-center">
            <div className="text-sm font-semibold text-white">{teacherName}</div>
            <div className="mt-0.5 text-xs text-white/40">{voiceState === "playing" ? "Speaking…" : phase === "playing" ? "Next slide…" : "Your tutor"}</div>
          </div>
          {lecture && (
            <div className="mt-6 w-full space-y-1 px-4">
              {lecture.slides.map((s, i) => (
                <button key={i} onClick={() => goToSlide(i)} className={`w-full rounded-lg px-3 py-2 text-left text-xs transition-colors ${i === slideIndex ? "bg-[var(--indigo)] text-white" : "text-white/40 hover:bg-white/10 hover:text-white/70"}`}>
        ...
        <div className="flex flex-1 flex-col">
          {phase === "loading" && (
            <div className="flex flex-1 flex-col items-center justify-center gap-4">
              ...
              <p className="text-white/60">Preparing your lecture on <strong className="text-white">{topicInput}</strong>…</p>
            </div>
          )}
          ...
          {phase === "error" && (
            <div className="flex flex-1 flex-col items-center justify-center gap-4">
              ...
              <button onClick={() => router.back()} className="rounded-full bg-white/10 px-5 py-2 text-sm hover:bg-white/20">Go back</button>
            </div>
          )}
          ...
          {phase === "ready" && !lecture && (
            <div className="flex flex-1 flex-col items-center justify-center gap-6 px-12">
              <div className="text-center">
                ...
                <p className="mt-2 text-white/50">Type any topic — your teacher will explain it slide by slide.</p>
              </div>
              <div className="flex w-full max-w-md flex-col gap-3">
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter" && topicInput.trim()) setPhase("loading"); }}
                  placeholder="e.g. Macbeth, The water cycle, World War 1, Quadratic equations…"
                  ...
                  autoFocus
                />
                <button onClick={() => { if (topicInput.trim()) setPhase("loading"); }} disabled={!topicInput.trim()} className="rounded-full bg-[var(--indigo)] px-6 py-3.5 text-[15px] font-semibold disabled:opacity-40 hover:opacity-90">
                  Generate lecture →
                </button>
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {["Macbeth", "The Water Cycle", "Quadratic Equations", "The French Revolution", "DNA and Genetics", "The Cold War", "Photosynthesis"].map((s) => (
                  <button key={s} onClick={() => { setTopicInput(s); setPhase("loading"); }} className="rounded-full border border-white/15 px-3.5 py-1.5 text-[13px] text-white/50 hover:border-white/30 hover:text-white/80">{s}</button>
                ))}
              </div>
            </div>
          )}
          ...
          {phase === "ready" && lecture && (
            <div className="flex flex-1 flex-col items-center justify-center gap-8 px-12">
              ...
              <button onClick={startLecture} className="flex items-center gap-3 rounded-full bg-gradient-to-r from-[var(--indigo-2)] to-[var(--indigo)] px-8 py-4 text-lg font-semibold hover:opacity-90">
          ...
          {(phase === "playing" || phase === "paused") && slide && (
            <div className="flex flex-1 flex-col px-12 py-8">
              <div className="mb-6">
                <div className="text-xs font-semibold uppercase tracking-widest text-[var(--indigo-2)]">Slide {slideIndex + 1} of {lecture?.slides.length}</div>
              ...
              <div className="space-y-3">
                {slide.points.map((point, i) => (
                  <div key={i} className="flex items-start gap-3 rounded-xl bg-white/5 px-5 py-3.5">
                    <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[var(--indigo)]/20 text-[11px] font-bold text-[var(--indigo-2)]">{i + 1}</span>
                    <p className="text-[15px] leading-relaxed text-white/90">{point}</p>
                  </div>
                ))}
              </div>
              {slide.example && (
                <div className="mt-6 rounded-2xl border border-[var(--gold)]/30 bg-[var(--gold)]/10 px-6 py-5">
                  <div className="mb-2 flex items-center gap-2">
                    <span className="text-lg">✏️</span>
                    <span className="text-[12px] font-bold uppercase tracking-wider text-[var(--gold)]">{slide.example.label}</span>
                  </div>
                  <p className="whitespace-pre-wrap font-mono text-[14px] leading-relaxed text-white/80">{slide.example.content}</p>
                </div>
              )}
              <div className={`mt-6 rounded-xl border px-5 py-4 text-[14px] leading-relaxed text-white/60 transition-all ${voiceState === "playing" ? "border-[var(--indigo)]/40 bg-[var(--indigo)]/10" : "border-white/10"}`}>
          ...
          {phase === "done" && lecture && (
            <div className="flex flex-1 flex-col items-center justify-center gap-6 px-12 text-center">
              ...
              <div className="flex gap-3">
                <button onClick={() => router.back()} className="rounded-full bg-[var(--indigo)] px-6 py-3 text-sm font-semibold hover:opacity-90">Go back</button>
              </div>
            </div>
          )}
        </div>
      ...
      {(phase === "playing" || phase === "paused") && (
        <div className="flex items-center justify-center gap-4 border-t border-white/10 py-4">
          <button onClick={prevSlide} disabled={slideIndex === 0} className="grid h-10 w-10 place-items-center rounded-full bg-white/10 disabled:opacity-30 hover:bg-white/20">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 20L9 12l10-8v16zM5 4h2v16H5z"/></svg>
          </button>
          {phase === "playing" ? (
            <button onClick={pause} className="grid h-14 w-14 place-items-center rounded-full bg-[var(--indigo)] hover:opacity-90">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
            </button>
          ) : (
            <button onClick={resume} className="grid h-14 w-14 place-items-center rounded-full bg-[var(--indigo)] hover:opacity-90">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M5 3l14 9-14 9V3z"/></svg>
            </button>
          )}
          <button onClick={nextSlide} disabled={!lecture || slideIndex >= lecture.slides.length - 1} className="grid h-10 w-10 place-items-center rounded-full bg-white/10 disabled:opacity-30 hover:bg-white/20">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M5 4l10 8-10 8V4zM17 4h2v16h-2z"/></svg>
          </button>
          <div className="ml-4 text-sm text-white/40">{phase === "paused" ? "Paused" : voiceState === "playing" ? `Speaking slide ${slideIndex + 1}…` : ""}</div>
        </div>
      )}
    </div>
  );
...
function TeacherAvatar({ name, isSpeaking, phase }: { name: string; isSpeaking: boolean; phase: Phase }) {
  return (
    <div className="relative flex flex-col items-center">
      {isSpeaking && <div className="absolute inset-0 animate-pulse rounded-full bg-[var(--indigo)]/20 blur-xl" />}
      ...
      <div className={`mt-2 h-2 w-2 rounded-full ${phase === "loading" ? "animate-pulse bg-yellow-400" : isSpeaking ? "animate-pulse bg-green-400" : "bg-white/30"}`} />
    </div>
  );
}
